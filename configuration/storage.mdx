...
When deploying to Vercel, use Vercel Blob for durable object storage. Add a Blob store to your Vercel project and Vercel will inject `BLOB_READ_WRITE_TOKEN` automatically.
...
Use the `s3` provider for AWS S3, Cloudflare R2, MinIO, or Railway object storage. All S3-compatible APIs are supported.
...
<AccordionGroup>
  <Accordion title="MinIO (Docker Compose)">
    ...
    Set `STORAGE_PUBLIC_URL_MODE=proxy` so that upload URLs are served through the store app (`/uploads/...`) rather than pointing directly to the MinIO container, which is not reachable from the browser.
  </Accordion>
...
| Value | Behavior |
| --- | --- |
| `direct` | Upload responses return the raw bucket or filesystem URL (e.g. `http://minio:9000/86d/stores/...`). This is the default. |
| `proxy` | Upload responses return a same-origin URL (e.g. `/uploads/stores/...`). The store app fetches the file from the backend and re-serves it to the browser. |
