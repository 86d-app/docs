...
description: "The admin dashboard at /admin gives you full control over your store. Modules register their own pages automatically into a 2-level sidebar."
---
...
The sidebar uses a 2-level collapsible structure. The top level has 9 fixed groups; each group contains subgroups that modules populate automatically.
...
For example, the `@86d-app/orders` module registers under **Sales → Orders**, and `@86d-app/blog` registers under **Marketing → Publishing** — without any configuration on your end.
...
- **300 requests/min per userId** for all `/api/admin/...` endpoints

If you exceed this limit, the API returns HTTP `429` with `Retry-After` and `X-RateLimit-Reset` headers.

## Dashboard overview

The admin dashboard (`/admin`) shows a summary of your store's current state, pulling live data from your enabled modules:
...
<Steps>
  ...
  <Step title="Navigate using the sidebar">
    Use the 2-level sidebar to reach any module's management pages. Click a group to expand it, then click a subgroup or page link.
  </Step>
...
<Tip>
  On mobile, the admin sidebar collapses into a hamburger menu. The same 2-level navigation structure is available in the mobile drawer.
</Tip>