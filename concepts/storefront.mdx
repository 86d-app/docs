...
The 86d storefront is a Next.js application that renders your store's public-facing pages. Every page is an MDX file from your active template. Module components — `<ProductGrid />`, `<Cart />`, `<NewsletterInline />` — are registered automatically when a module is enabled, so you place them in MDX the same way you place any other element. No import statements, no boilerplate.
...
<AccordionGroup>
  <Accordion title="Public endpoints — 120 req/min per IP">
  ...
  <Accordion title="Sensitive endpoints — 10 req / 10 min per IP">
    Endpoints that could be abused for spam or fraud — including newsletter subscribe and payment intent creation — have a stricter limit of 10 requests per 10 minutes per IP.
  </Accordion>
...
<Tip>
  To see all components available in your current setup, run `bun run generate:docs`. This generates `docs/component-api.md` with the full list of registered components and their props.
</Tip>