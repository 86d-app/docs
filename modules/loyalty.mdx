...
The loyalty module gives your customers a points-based rewards program with four built-in tiers (bronze, silver, gold, platinum), configurable earning rules, and automatic point accrual whenever an order is placed. Accounts are provisioned automatically on a customer's first interaction — no manual setup required.
