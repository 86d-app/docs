...
The discounts module handles promo code creation, validation, and application. It is standalone — it has no dependencies on other modules and requires no configuration at initialization time. Discount rules and promo codes are managed entirely at runtime through the admin endpoints or the controller API. The checkout module integrates with discounts automatically via structural typing, so promo codes entered in `CheckoutSummary` flow through without any additional wiring on your part.
...
interface Discount {
  ...
  description?: string;
  type: DiscountType;
  value: number;                // percentage 0–100, cents, or 0 for free_shipping
