...
<ParamField path="webhookSecret" type="string">
  Webhook signing secret from the Stripe dashboard (`whsec_...`). When provided, every incoming webhook request is verified using HMAC-SHA256 with a 5-minute timestamp tolerance. Without this value, all webhook requests are accepted — useful for local development but not suitable for production.
</ParamField>
