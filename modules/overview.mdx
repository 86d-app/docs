...
description: "Browse all 100+ modules available in the 86d commerce platform, organized by category. Each module is an isolated npm package you install independently."
---

86d is built around a modular architecture where every feature ships as a standalone package. You install only the modules your store needs, configure them in `config.json`, and they become available as endpoints and MDX components in your storefront. All modules are published to npm under the `@86d-app` scope and share the same version number.

## Installation

Install any module with npm:

```sh
npm install @86d-app/<module-name>
```

Then add it to your store's `config.json` under the `modules` array. Once registered, the module's API endpoints and store components are automatically available.
...
<AccordionGroup>
  <Accordion title="Catalog">
    ...
    | product-labels | `@86d-app/product-labels` | Visual badges — "New", "Sale", "Best Seller", etc. |
...
- [Products module reference](/modules/products) — the core catalog module
- [Collections module reference](/modules/collections) — curated product groupings
- [Brands module reference](/modules/brands) — manufacturer and brand pages
- [Media module reference](/modules/media) — digital asset management